...
const API_CONFIG = {
  ...
  productsPath: "/products",       // GET {baseUrl}{productsPath}  -> lista de produtos
  productPath: id => `/products/${id}`, // GET {baseUrl}{productPath(id)} -> um produto (opcional, hoje não é usado: o site já busca a lista toda e filtra localmente)
  timeoutMs: 5000
};
...
async function fetchProductsFromApi() {
  ...
  try {
    const res = await fetch(API_CONFIG.baseUrl + API_CONFIG.productsPath, {
      method: "GET",
    ...
    if (!Array.isArray(data) || data.length === 0) throw new Error("Resposta da API vazia ou em formato inesperado");
    return data;
  } catch (err) {
    clearTimeout(timeout);
    throw err;
